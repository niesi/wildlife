    ...
    try:
        while True:
            ...
            if tracker.active:
                ...
                if ok_track:
                    draw_result(frame, box, tracker.label, None, (0, 200, 255))
            ...
            if not tracker.active:
                for box in boxes:
                    ...
                    draw_result(frame, box, result.label, result.confidence, color)
                    if result.is_animal:
                        tracker.start(frame, box, result.label)
                        break  # nur ein Objekt gleichzeitig verfolgen (Start)

            cv2.imshow("Wildlife Pipeline (Prototyp)", frame)
